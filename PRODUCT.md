...
The first native prototype proves the iPhone setup/capture and multimodal coaching loop: camera permission, device-level feedback, four guided still captures, push-to-talk audio, OMNI image+audio reasoning, OMNI speech playback, captions, and local session history. Numeric posture findings appear only after the existing measurement engine is connected to a verified native pose pipeline. Until then, the app shows capture status and provider-backed coaching without invented metrics.
