import { modelGuidanceDraftSchema, type ModelGuidanceDraft } from "@align/contracts";
...
const STANDARD_LIMITATIONS = [
  ...
  "No diagnosis or verified posture angle was produced.",
  "Stop any suggested movement that causes pain, dizziness, numbness, or weakness.",
];

export function validateGuidanceDraft(input: unknown) {
  ...
  if (numericFinding.test(allText)) throw new Error("invented_numeric_finding");
  ...
  return {
    ...
    limitations: [...new Set([...draft.limitations, ...STANDARD_LIMITATIONS])],
    safety,
    sources: hasUrgent
